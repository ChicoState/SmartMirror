...
function loadData(system) {
    ...
    var accuweatherApiKey = "eOYiiAjNR0EuRaIGNoxAlXQQLn56cQMb"; // Accuweather api key1
    ...
    var location = result[0].EnglishName; // City name
      ...
      */
      forecastResourceURL = 'https://dataservice.accuweather.com/forecasts/v1/daily/5day/' + locationKey + '?apikey=' + accuweatherApiKey + '&details=true' + '&metric=false';
      hourlyForecastResourseURL = 'https://dataservice.accuweather.com/forecasts/v1/hourly/12hour/' + locationKey + '?apikey=' + accuweatherApiKey + '&metric=false';
      ...
      $.ajax({
        ...
        for (var i = 0; i < 12; i++) {
          ...
          var hourlyTemp = Math.round(result[i].Temperature.Value).toString() + ' °' +result[i].Temperature.Unit.toString();
